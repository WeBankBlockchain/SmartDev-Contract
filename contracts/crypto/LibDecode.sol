...
library LibDecode {
    function decode(bytes signhash, bytes signedString) returns (address){
    ...
    function ecrecoverDecode(bytes32 hash, bytes32 r, bytes32 s, byte v1) returns (address addr){
        uint8 v = uint8(v1) + 27;
