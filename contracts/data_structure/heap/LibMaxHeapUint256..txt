...
function insert(Heap storage heap, uint256 value) internal{
    ...
    while(idx > 0){
        ...
        if(heap.data[parent] >= value){ // 如果父节点的值大于等于新元素的值，则跳出循环
            break;
        }
        // 做上移操作
...
function extractTop(Heap storage heap) internal returns(uint256 top){
    ...
    while(index < heap.data.length){
        ...
        swapValue = last;
        swapIndex = index;
        if(leftIdx < heap.data.length && swapValue < heap.data[leftIdx]){ // 如果左子节点的值大于当前值，则进行交换
            swapValue = heap.data[leftIdx];
            swapIndex = leftIdx;
        }

        if(rightIdx < heap.data.length && swapValue < heap.data[rightIdx]){ // 如果右子节点的值大于当前值，则进行交换
