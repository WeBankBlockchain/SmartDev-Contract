...
- 最高权限者调用`addAuthUser`添加具体业务权限管理者`manager`。
- `manager`通过平台业务系统接收共享充电服务商 提交充电设备区块链账户地址，调用`setAuthDevice`方法进行合约设备授权。
- 业务系统在用户完成注册，并且完成实名制后，签发区块链公私钥及账户地址，由`manager`管理者通过调用`addAuthUser`方法，将用户基础信息注册到合约中。
- 在实际场景中，用户在充电设备端提供的二维码进行扫码充电；完成充电后，进行付款，此时用户app会自动为该笔交进行一次签名并同步平台，而充电设备会得到本次交易订单和用户签名，由充电设备调用`addEvidence`方法进行存证，同时用户获得对应的能量积分`积分比例由业务端设定，授权充电设备通过对应比例将积分数值存入交易`。
